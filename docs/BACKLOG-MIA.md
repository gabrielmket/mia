...
| E4 | **Ligar agregação de logs** na VPS | pequeno | **do Gabriel, e não meu — conferido em 19/09.** O MCP do EasyPanel tem `updateLogsSettings`, e o estado hoje é desligado (`getLogsSettings` devolve nulo, `getLogsStats` dá uso zero). Não liguei: isso instala Loki e Promtail **no servidor inteiro**, fora da fronteira `clientes/deskcomm`, numa VPS com CPU saturada rodando produção de outros clientes (n8n, Supabase, calcom). Custa CPU e disco de todo mundo — é decisão dele. Enquanto estiver desligado, log de contêiner efêmero não existe: foi o que me impediu de ler o bootstrap depois do deploy, e o que motivou o carimbo do schema (0268/0269) como substituto parcial. |
| E5 | ~~Desligar o workflow `release`~~ | — | feito em 18/09 |
| ~~E7~~ | ~~**A página legal nomeia o CLIENTE como controlador**~~ | — | feito na `.42` (migration 0267). `platform_branding` ganhou razão social, CNPJ, encarregado e política do OPERADOR da instalação, e `operador_razao_social` é o interruptor: nula segue self-host (sai da sessão, como sempre), preenchida vale para todo leitor — com sessão ou sem. Interruptor implícito e não uma coluna `modo`, porque um modo explícito pode ficar em 'gerenciado' com a razão social vazia e aí a página diria "o controlador é " e pararia. Preenche-se em **Admin › Marca › Responsável legal desta instalação**. Leitura que FALHA cai no documento genérico e não na sessão: voltar a nomear o cliente em silêncio é exatamente o defeito. |
| E6 | **MCP de administração da plataforma** | médio | **no ar** (migration 0271). `/api/mcp/plataforma`, token `dskp_` em tabela própria, tela em **Admin › Tokens de plataforma**. LER é livre: `listar_clientes`, `ver_cliente`, `listar_modulos`, `ver_saude` (o estado do schema com a amostra do erro, canais fora do ar e crédito de IA). ESCREVER é nomeado: `criar_cliente`, `liberar_modulo`, `lancar_credito`, `definir_preco`. Token sem nenhuma marcada só lê; não existe curinga, e há teste reprovando `*`, `all`, `admin`, `full` e `tudo`. **O que NÃO entrou, e o motivo:** `convidar_pessoa` entrega dado de terceiro e é a mais difícil de desfazer — quer o fluxo de convite atravessado com o cuidado que a tela já tem. `criar_template` e `conectar_canal` **não são operações de plataforma**: a primeira é de TENANT (usa as credenciais Meta daquele cliente) e a segunda termina num QR que só um humano lê numa tela autenticada. Embrulhar as duas aqui criaria um segundo jeito de fazer a mesma coisa, que diverge no primeiro conserto — é o risco que o docstring de `ferramentas.ts` descreve. |
| ~~E8~~ | ~~**Testes que falham por CARGA, não por defeito**~~ | — | fechado na `.53`, em três passos e com duas correções de rumo minhas. (1) Um era defeito claro: `drain-loop` dava 60s ao processo filho e o vitest o matava aos 15. (2) Declarei tempo por arquivo nos quatro seguintes — e **ficou pela metade**: só `testTimeout`, quando o custo mora no `beforeEach`, e `hookTimeout` tem orçamento próprio. (3) Medido que eram **sete** arquivos em seis rodadas, todos passando isolados, a causa era estrutural e não excepcional: `hookTimeout` nunca tinha sido declarado (ficou no padrão de 10s, MENOR que o dos testes) e a suíte passou de 842 arquivos, indo de ~390s para 774-813s sob disputa. Os dois tetos agora são 30s no `vitest.config.ts`, com a medição escrita. Travamento continua reprovando — só 15s depois. |
...
|---|---|---|---|
| ~~G1~~ | ~~**Conectar o número pela própria tela do admin** (QR ali dentro)~~ | — | **já estava fechado** e a linha é que estava velha (a terceira do bloco G, junto com G2 e G4 — vale reler o bloco inteiro antes de abrir item daqui). `/admin/numero-de-avisos` tem o botão "Conectar e mostrar o QR" e renderiza o código na própria tela, atualizando a cada 3s porque ele expira em segundos. Está inclusive listado na tabela de entregues logo acima. |
| ~~G2~~ | ~~**Aviso quando o número de avisos cai** chegando a quem OPERA~~ | — | **já estava fechado** e a linha é que estava velha: o report da plataforma (cron `report-da-plataforma`) avisa o grupo INTERNO quando qualquer número cai, e tem texto próprio, mais alto, quando o que caiu é o número de avisos ("Enquanto ele estiver fora, NENHUM cliente recebe aviso de lead qualificado no grupo"). Quem opera é exatamente quem lê aquele grupo. |
...
| ~~G4~~ | ~~**Tirar o número de avisos do seletor de atendimento**~~ | — | feito na `.51`. `listSelectableChannels` é a fonte única dos seletores de "Número conectado" e passou a filtrá-lo. Ele **continua** em Conexões da organização que o conectou — é por ali que se lê o QR e se reconecta. O filtro FALHA ABERTA de propósito: `e_numero_de_avisos` veio de migration, e num banco sem ela um `.neq()` derrubaria a consulta e o editor de agente pararia de abrir. Tolera-se o que, dando errado, devolve a lista de ANTES do filtro — nunca o que a esvazia. |
