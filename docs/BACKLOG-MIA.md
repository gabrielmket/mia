...
| E6 | **MCP de administração da plataforma** | médio | **primeira metade no ar** (migration 0271). Servidor próprio em `/api/mcp/plataforma`, token `dskp_` em tabela própria, e a tela em **Admin › Tokens de plataforma**. LER é livre (listar clientes, ver um cliente, listar módulos); ESCREVER é nomeado, uma operação por vez: `criar_cliente`, `liberar_modulo`, `lancar_credito`, `definir_preco`. Token sem nenhuma marcada só lê. Não existe curinga — há teste que reprova `*`, `all`, `admin`, `full` e `tudo`. **Falta:** `convidar_pessoa` (deixada de fora de propósito: entrega dado de terceiro e é a mais difícil de desfazer — quer o fluxo de convite atravessado com o cuidado que a tela já tem), conectar canal e criar template. |
