...
| E8 | **Testes que falham por CARGA, não por defeito** | pequeno | medido em 19/09: três rodadas da suíte completa com a máquina disputada (774s, 813s e 442s contra ~390s normais) reprovaram **conjuntos diferentes** de 2-3 arquivos, todos passando isolados, todos com relógio real acima de 2s (`theme`, `channel-health-aviso`, `agenda-meet`, `agenda-meet-routes`, `drain-loop-carrega-deps-sob-tsx`). Um já foi consertado na raiz (`empresa-no-card` comparava dois cards construídos em instantes diferentes — relógio preso). Os outros esperam relógio de verdade e vão reprovar no CI num dia movimentado, com a mensagem apontando para o lugar errado. O conserto é por arquivo: prender relógio, ou declarar o tempo com motivo escrito, como `hidratacao-useState` já faz. |
